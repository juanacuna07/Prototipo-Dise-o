...
/**
 * Permite consultar un AgenteNavegacion seleccionandolo de una lista visible,
 * sin tener que recordar ni escribir su ID.
 *
 * @author Juan Acuña, Luis Hernández, Stephany Trujillo
 */
public class GUIConsultarNavegacion extends javax.swing.JFrame {

    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(GUIConsultarNavegacion.class.getName());

    private JComboBox<AgenteInteligente> cmbAgentes;
    private JLabel lblNombre, lblAutonomia, lblEstado, lblConfianza, lblOrigen, lblDestino, lblObstaculos, lblPrecision;

    public GUIConsultarNavegacion() {
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("Consultar Agente de Navegacion");

        JPanel panel = new JPanel(new GridLayout(0, 2, 8, 8));
        panel.setBackground(new Color(21, 101, 192));
        panel.setBorder(javax.swing.BorderFactory.createEmptyBorder(15, 15, 15, 15));

        JLabel lblTitulo = new JLabel("Consultar Agente de Navegacion");
        ...
        lblOrigen = agregarFila(panel, "Origen:");
        lblDestino = agregarFila(panel, "Destino:");
        lblObstaculos = agregarFila(panel, "Obstaculos Detectados:");
        lblPrecision = agregarFila(panel, "Precision GPS:");
    ...
    private void cargarAgentes() {
        ServicioAgentes.getAgentes().values().stream()
                .filter(AgenteNavegacion.class::isInstance)
    ...
    private void mostrarSeleccionado() {
        ...
        if (!(seleccionado instanceof AgenteNavegacion ag)) {
            limpiarDatos();
            return;
        }
    ...
    private void limpiarDatos() {
        ...
        lblOrigen.setText("-");
        lblDestino.setText("-");
        lblObstaculos.setText("-");
        lblPrecision.setText("-");
        if (cmbAgentes.getItemCount() == 0) {
            JOptionPane.showMessageDialog(this, "No hay agentes de navegacion registrados.", "Sin agentes", JOptionPane.INFORMATION_MESSAGE);
        }
    }
}
